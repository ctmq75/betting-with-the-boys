...
class FutureGamePredictor:
    ...
    def create_upcoming_match_features(self, home_team, away_team, game_week=20):
        ...
        home_stats = self.team_stats.get(
            home_team,
            {"ppg": 1.0, "goals_per_game": 1.0, "goals_against_per_game": 1.0},
        )
        ...
        features["Pre-Match PPG (Home)"] = (
            home_stats["ppg"] * FEATURE_WEIGHTS["historical_form"]
        )
        features["Pre-Match PPG (Away)"] = (
            away_stats["ppg"] * FEATURE_WEIGHTS["historical_form"]
        )
        features["pre_match_ppg_difference"] = (
            features["Pre-Match PPG (Home)"] - features["Pre-Match PPG (Away)"]
        )
        ...
        home_attack_strength = (
            home_stats["goals_per_game"] * FEATURE_WEIGHTS["attacking"]
        )
        away_attack_strength = (
            away_stats["goals_per_game"] * FEATURE_WEIGHTS["attacking"]
        )

        # Estimated shots based on attacking strength
        features["home_team_shots"] = (10 + home_attack_strength * 3) * FEATURE_WEIGHTS[
            "home_advantage"
        ]
        ...
        team_strength_diff = (home_stats["ppg"] - away_stats["ppg"]) * FEATURE_WEIGHTS[
            "recent_form"
        ]
        base_possession = 50
        possession_adjustment = team_strength_diff * 8  # Each PPG point = 8% possession
        home_advantage_possession = (
            3 * FEATURE_WEIGHTS["home_advantage"]
        )  # Home gets extra 3%

        features["home_team_possession"] = (
            min(
                70,
                max(
                    30,
                    base_possession + possession_adjustment + home_advantage_possession,
                ),
            )
            * FEATURE_WEIGHTS["possession"]
        )
        features["away_team_possession"] = (
            100 - features["home_team_possession"]
        ) * FEATURE_WEIGHTS["possession"]

        # WEIGHTED SET PIECES (corners)
        features["home_team_corner_count"] = (
            (4 + home_attack_strength)
            * FEATURE_WEIGHTS["set_pieces"]
            * FEATURE_WEIGHTS["home_advantage"]
        )
        features["away_team_corner_count"] = (
            4 + away_attack_strength
        ) * FEATURE_WEIGHTS["set_pieces"]

        # Cards and fouls - slightly weighted by aggression/form
        aggression_factor = (
            1 + abs(team_strength_diff) * 0.1
        )  # Closer games = more cards
        ...
        features["home_win_probability"] = (1 / home_odds) * FEATURE_WEIGHTS[
            "recent_form"
        ]
        features["draw_probability"] = 1 / draw_odds
        features["away_win_probability"] = (1 / away_odds) * FEATURE_WEIGHTS[
            "recent_form"
        ]
        features["total_probability"] = (
            features["home_win_probability"]
            + features["draw_probability"]
            + features["away_win_probability"]
        )
        ...
        home_defense_strength = 2.0 / max(
            0.5, home_stats.get("goals_against_per_game", 1.0)
        )
        away_defense_strength = 2.0 / max(
            0.5, away_stats.get("goals_against_per_game", 1.0)
        )
...
def predict_match_integrated(self, home_team, away_team, game_week=20):
    ...
    match_features = self.create_upcoming_match_features(
    ...
    try:
        ...
        integrated_predictions = self.integrate_predictions(
            ml_predictions, goal_predictions
        )
        ...
        predictions["predictions"] = {
            ...
            "btts_prob": 0.5,
            "btts_no_prob": 0.5,
            "home_predicted_goals": 1,
            "away_predicted_goals": 1,
    ...
    def validate_prediction_logic(self, predictions):
        ...
        if total_goals >= 3 and over_25_prob < 0.6:
            issues.append(
                f"❌ Predicted {total_goals} goals but only {over_25_prob:.1%} Over 2.5"
            )
        elif total_goals <= 2 and over_25_prob > 0.6:
            issues.append(
                f"❌ Predicted {total_goals} goals but {over_25_prob:.1%} Over 2.5"
            )
        ...
        if home_goals > 0 and away_goals > 0 and btts_prob < 0.5:
            issues.append(
                f"❌ Both teams score ({home_goals}-{away_goals}) but BTTS only {btts_prob:.1%}"
            )
        elif (home_goals == 0 or away_goals == 0) and btts_prob > 0.5:
            issues.append(
                f"❌ Clean sheet predicted ({home_goals}-{away_goals}) but BTTS {btts_prob:.1%}"
            )
        ...
        if home_goals > away_goals and most_likely != "Home Win":
            issues.append(
                f"❌ Home wins on score ({home_goals}-{away_goals}) but {most_likely} most likely"
            )
        elif away_goals > home_goals and most_likely != "Away Win":
            issues.append(
                f"❌ Away wins on score ({home_goals}-{away_goals}) but {most_likely} most likely"
            )
        elif home_goals == away_goals and most_likely != "Draw":
            issues.append(
                f"❌ Draw on score ({home_goals}-{away_goals}) but {most_likely} most likely"
            )
    ...
    def generate_integrated_betting_recommendations(
        self, predictions, match_features, confidence_threshold=0.65
    ):
        ...
        if "confidence" in preds and preds["confidence"] >= confidence_threshold:
            if (
                preds["most_likely"] == "Home Win"
                and preds["home_win_prob"] >= confidence_threshold
            ):
                ...
                preds["most_likely"] == "Away Win"
                and preds["away_win_prob"] >= confidence_threshold
            ):
                recommendations.append(
                    {
                        "bet_type": "Away Win",
                ...
                preds["most_likely"] == "Draw"
                and preds["draw_prob"] >= confidence_threshold
            ):
                recommendations.append(
                    {
                        "bet_type": "Draw",
                ...
                recommendations.append(
                    {
                        ...
                        "strength": (
                            "High" if preds["over_2.5_prob"] > 0.8 else "Medium"
                        ),
                    }
                )
                ...
                recommendations.append(
                    {
                        ...
                        "strength": (
                            "High" if preds["under_2.5_prob"] > 0.8 else "Medium"
                        ),
                    }
                )
    ...
    def predict_multiple_matches(self, matches_list, use_integrated=True):
        ...
        print(
            f"🔮 Predicting {len(matches_list)} matches using {method_name} method..."
        )
        ...
        for i, match in enumerate(matches_list, 1):
            ...
            if use_integrated:
                prediction = self.predict_match_integrated(
                    home_team, away_team, game_week
                )
                if prediction:
    ...
    def predict_match(self, home_team, away_team, game_week=20):
        ...
        try:
            ...
            if "outcome_rf" in self.trained_model.models:
                outcome_probs = self.trained_model.models["outcome_rf"].predict_proba(
                    match_features
                )[0]
                ...
                predictions["predictions"]["confidence"] = round(
                    outcome_probs[most_likely_idx], 3
                )

            # Goals predictions
            if "over_2.5_rf" in self.trained_model.models:
                over25_prob = self.trained_model.models["over_2.5_rf"].predict_proba(
                    match_features
                )[0][1]
            ...
            if "btts_rf" in self.trained_model.models:
                btts_prob = self.trained_model.models["btts_rf"].predict_proba(
                    match_features
                )[0][1]
    ...
    def predict_team_goals_legacy(self, predictions, home_team, away_team):
        ...
        away_stats = self.team_stats.get(
            away_team, {"goals_per_game": 1.2, "goals_against_per_game": 1.2}
        )
        ...
        away_expected_goals = (away_attack * away_attack_weight) + (
        ...
        predictions["predictions"]["home_expected_goals"] = round(
            home_expected_goals, 2
        )
        predictions["predictions"]["away_expected_goals"] = round(
            away_expected_goals, 2
        )
        predictions["predictions"][
            "predicted_score"
        ] = f"{home_predicted_goals}-{away_predicted_goals}"
        predictions["predictions"]["total_predicted_goals"] = (
            home_predicted_goals + away_predicted_goals
        )
        predictions["predictions"]["total_expected_goals"] = round(
            home_expected_goals + away_expected_goals, 2
        )

    def generate_betting_recommendations(
        self, predictions, match_features, confidence_threshold=0.6
    ):
    ...
    def save_predictions(self, predictions):
        ...
        with open(txt_file, "w") as f:
            ...
            f.write(
                f"Prediction Method: Integrated (ML + Goal-based with consistency checks)\n\n"
            )
            ...
            for i, pred in enumerate(predictions, 1):
                ...
                if "predictions" in pred:
                    ...
                    f.write(
                        f"🎯 Most Likely: {p.get('most_likely', 'Unknown')} ({p.get('confidence', 0):.1%} confidence)\n\n"
                    )

                    # Goals predictions
                    if "predicted_score" in p:
                        f.write(
                            f"⚽ Predicted Score: {p.get('predicted_score', 'N/A')}\n"
                        )
                        f.write(
                            f"🏠 Home: {p.get('home_predicted_goals', 0)} goals ({p.get('home_expected_goals', 0)} xG)\n"
                        )
                        f.write(
                            f"✈️  Away: {p.get('away_predicted_goals', 0)} goals ({p.get('away_expected_goals', 0)} xG)\n"
                        )
                        f.write(
                            f"📊 Total: {p.get('total_predicted_goals', 0)} goals ({p.get('total_expected_goals', 0)} xG)\n\n"
                        )

                    # Betting markets
                    f.write(
                        f"⚽ Over 2.5: {p.get('over_2.5_prob', 0):.1%} | Under 2.5: {p.get('under_2.5_prob', 0):.1%}\n"
                    )
                    f.write(
                        f"🥅 BTTS Yes: {p.get('btts_prob', 0):.1%} | BTTS No: {p.get('btts_no_prob', 0):.1%}\n\n"
                    )
                ...
                if pred.get("betting_recommendations"):
                    ...
                    for bet in pred["betting_recommendations"]:
                        ...
                        f.write(
                            f"   {strength_emoji} {bet['bet_type']} - {bet['probability']:.1%} confidence\n"
                        )
...
def predict_next_weekend():
    ...
    predictions = predictor.predict_multiple_matches(
        upcoming_matches, use_integrated=True
    )
    ...
    for pred in predictions:
        ...
        if "predictions" in pred:
            p = pred["predictions"]
            print(
                f"   Most likely: {p.get('most_likely', 'Unknown')} ({p.get('confidence', 0):.1%})"
            )
            ...
            if pred.get("betting_recommendations"):
                ...
                print(
                    f"   Recommended bets: {total_bets} total, {high_conf_count} high confidence"
                )
                ...
                if high_conf_bets:
                    ...
                    print(
                        f"   🔥 Best bet: {best_bet['bet_type']} ({best_bet['probability']:.1%})"
                    )
    ...
    print(
        f"   Average confidence per match: {total_recommendations/len(predictions):.1f} bets"
    )
...
if __name__ == "__main__":
    ...
    def get_ml_baseline_predictions(self, match_features):
        ...
        try:
            ...
            # Goals predictions
            if "over_2.5_rf" in self.trained_model.models:
                ml_preds["ml_over_2.5_prob"] = self.trained_model.models[
                    "over_2.5_rf"
                ].predict_proba(match_features)[0][1]

            # BTTS prediction
            if "btts_rf" in self.trained_model.models:
                ml_preds["ml_btts_prob"] = self.trained_model.models[
                    "btts_rf"
                ].predict_proba(match_features)[0][1]
            ...
            ml_preds = {
                "ml_home_win_prob": 0.4,
                "ml_draw_prob": 0.3,
                "ml_away_win_prob": 0.3,
                "ml_over_2.5_prob": 0.5,
                "ml_btts_prob": 0.5,
            }
    ...
    def predict_team_goals_improved(self, home_team, away_team):
        ...
        home_stats = self.team_stats.get(
            home_team,
            {"goals_per_game": 1.2, "goals_against_per_game": 1.2, "ppg": 1.5},
        )
        away_stats = self.team_stats.get(
            away_team,
            {"goals_per_game": 1.2, "goals_against_per_game": 1.2, "ppg": 1.5},
        ...
        away_expected_goals = (away_attack * away_attack_weight) + (
            (1.8 - home_defense) * home_defense_weight
        )  # REDUCED from 2.5 to 1.8
        ...
        home_expected_goals = max(
            0.3, min(2.5, home_expected_goals)
        )  # REDUCED max from 3.5 to 2.5
        away_expected_goals = max(
            0.3, min(2.5, away_expected_goals)
        )  # REDUCED max from 3.5 to 2.5
    ...
    def integrate_predictions(self, ml_predictions, goal_predictions):
        ...
        integrated["home_win_prob"] = round(
            ...
            + (goal_predictions["goal_based_home_win_prob"] * GOAL_WEIGHT),
            3,
        )

        integrated["draw_prob"] = round(
            (ml_predictions.get("ml_draw_prob", 0.33) * ML_WEIGHT)
            + (goal_predictions["goal_based_draw_prob"] * GOAL_WEIGHT),
            3,
        )

        integrated["away_win_prob"] = round(
            (ml_predictions.get("ml_away_win_prob", 0.33) * ML_WEIGHT)
            + (goal_predictions["goal_based_away_win_prob"] * GOAL_WEIGHT),
            3,
        )

        # Normalize probabilities to sum to 1.0
        total_prob = (
            integrated["home_win_prob"]
            + integrated["draw_prob"]
            + integrated["away_win_prob"]
        )
        ...
        integrated["over_2.5_prob"] = round(
            ...
            + (goal_predictions["goal_based_over_2_5_prob"] * GOAL_WEIGHT),
            3,
        )
        ...
        integrated["btts_prob"] = round(
            ...
            + (goal_predictions["goal_based_btts_prob"] * GOAL_WEIGHT),
            3,
        )
        ...
        integrated["predicted_score"] = (
            f"{goal_predictions['home_predicted_goals']}-{goal_predictions['away_predicted_goals']}"
        )
        ...
        integrated["total_predicted_goals"] = (
            goal_predictions["home_predicted_goals"]
            + goal_predictions["away_predicted_goals"]
        )
    ...
    def apply_consistency_checks(self, predictions):
        ...
        if predictions["total_predicted_goals"] >= 3:
            ...
            print(
                f"✅ Consistency fix: Predicted {predictions['total_predicted_goals']} goals, boosted Over 2.5 to {predictions['over_2.5_prob']:.1%}"
            )

        # Check 2: If both teams predicted to score, BTTS should be high
        if (
            predictions["home_predicted_goals"] > 0
            and predictions["away_predicted_goals"] > 0
        ):
            predictions["btts_prob"] = max(0.65, predictions["btts_prob"])
            predictions["btts_no_prob"] = 1 - predictions["btts_prob"]
            print(
                f"✅ Consistency fix: Both teams score in prediction, boosted BTTS to {predictions['btts_prob']:.1%}"
            )

        # Check 3: If one team predicted to score 0, BTTS should be low
        elif (
            predictions["home_predicted_goals"] == 0
            or predictions["away_predicted_goals"] == 0
        ):
            predictions["btts_prob"] = min(0.35, predictions["btts_prob"])
            predictions["btts_no_prob"] = 1 - predictions["btts_prob"]
            print(
                f"✅ Consistency fix: Clean sheet predicted, reduced BTTS to {predictions['btts_prob']:.1%}"
            )
        ...
        if home_goals > away_goals:  # Home win predicted
            if predictions["most_likely"] != "Home Win":
                print(
                    f"✅ Consistency fix: Score predicts home win ({home_goals}-{away_goals}), adjusting outcome"
                )
            ...
            if predictions["most_likely"] != "Away Win":
                print(
                    f"✅ Consistency fix: Score predicts away win ({home_goals}-{away_goals}), adjusting outcome"
                )
            ...
            if predictions["most_likely"] != "Draw":
                print(
                    f"✅ Consistency fix: Score predicts draw ({home_goals}-{away_goals}), adjusting outcome"
                )
